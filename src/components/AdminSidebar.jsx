  ...
  return (
    <div className="bg-gray-900 dark:bg-gray-950 text-white w-64 min-h-screen flex flex-col transition-colors duration-300">
      <div className="p-6 border-b border-gray-700 dark:border-gray-800">
        ...
        <p className="text-gray-400 text-sm mt-1 animate-fadeInLeft animation-delay-200">NewsHub Management</p>
      </div>
      
      <nav className="flex-1 px-4 py-6">
        <ul className="space-y-2">
          {menuItems.map((item, index) => (
            <li key={item.path} className="animate-fadeInLeft" style={{ animationDelay: `${(index + 1) * 100}ms` }}>
              <Link
                to={item.path}
